...
cimport cudf._lib.includes.groupby.common as common_groupby
cimport cudf._lib.includes.groupby.hash as hash_groupby
cimport cudf._lib.includes.groupby.sort as sort_groupby

agg_names = {
    'sum': common_groupby.SUM,
    'min': common_groupby.MIN,
    'max': common_groupby.MAX,
    'count': common_groupby.COUNT,
    'mean': common_groupby.MEAN
}
...
def groupby(
    ...
    cdef vector[common_groupby.operators] c_ops
